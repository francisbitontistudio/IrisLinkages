...
def Main():
    ...
    crvs=setup[0]
    intersects=setup[1]
    arms=attachArms(arm,joint,crvs,ref,holeRad,gap,rows)
    subtractLink=rs.AddCylinder(rs.CurveStartPoint(ref),5,holeRad)
    holes=attachLinks(subtractLink,subtractLink,crvs,intersects,ref,rows)
    holes[0].extend(holes[1])
    links=attachLinks(bigLink,smallLink,crvs,intersects,ref,rows)
